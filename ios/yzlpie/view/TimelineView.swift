...
public class TimelineView: UIView {
	...
	*/
    public init(bulletType: BulletType, timeFrames: [TimeFrame], currentController: UIViewController){
	...
	private func blockForTimeFrame(element: TimeFrame, imageTag: Int) -> UIView{
		...
		if let image = element.image{
            ...
            backgroundViewForImage.alpha = 0.2
			...
			button.tag = imageTag
            button.titleLabel?.text = element.url
    ...
    func tapImage(sender: UIButton){
        let printDetailController = FootPrintDetailController()
        printDetailController.url = sender.titleLabel?.text
        self.controller?.hidesBottomBarWhenPushed = true
        self.controller?.navigationController?.pushViewController(printDetailController, animated: true)
